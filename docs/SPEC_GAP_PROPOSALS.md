...
**Status:** `OWNER_ACCEPTED` — encoded into a proposed `SUAS-specs` **0.1.4** conformance-codification release, now open as a canonical PR.
**Canonical PR:** [scrimshawlife-ctrl/SUAS-specs#7](https://github.com/scrimshawlife-ctrl/SUAS-specs/pull/7) — branch `cursor/spec-0.1.4-conformance-codification-1036` (19 files, +215/-44), pushed via the GitHub app after write access was granted and verified **byte-identical** to the reviewed patch. Merging that PR ratifies the 0.1.4 release (`SUAS-specs` lifecycle is owner-controlled).
**Delivery (backup):** `docs/spec-patches/SUAS-specs-0.1.4-conformance-codification.patch` (git `am`-applyable, 5 commits, verified to apply cleanly on `SUAS-specs@main` `33c6f19`) remains as an offline copy. The P-12 `subject_type` and P-13 `destination_type` enum casing matches the values the merged implementation actually stores (`ServiceRequest`, `Organization` — the canonical PascalCase entity-type names), so ratifying introduces no spec↔code drift.
